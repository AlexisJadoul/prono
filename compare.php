...
$uname = isset($_GET['u']) ? trim($_GET['u']) : (current_user()?:'');

$st=$pdo->prepare("SELECT id,username FROM users WHERE username=?"); $st->execute([$uname]); $user=$st->fetch(PDO::FETCH_ASSOC);
if(!$user){ echo "<div class='card'><h2>Comparaison classement</h2><p class='err'>Utilisateur inconnu</p></div>"; require_once __DIR__.'/footer.php'; exit; }
$uid=(int)$user['id'];
...
?>
<div class="card">
  <h2>Comparaison a date - <?=h($user['username'])?></h2>
  <p class="muted">Date de coupe: <strong><?=h($cut)?></strong>. Δ rang = reel - predit.</p>
  <p style="margin-top:6px">
    <a class="badge" href="compare.php?u=<?=urlencode($user['username'])?>&cut=now">Coupe maintenant</a>
    <a class="badge" href="compare_md.php?u=<?=urlencode($user['username'])?>">Comparer par journee</a>
  </p>
</div>
<?php if(!$rows): ?>
  <div class="card"><p class="muted">Pas de donnees.</p></div>
<?php else: ?>
  <div class="card">
    <table>
      <tr><th>Equipe</th><th>Rang predit</th><th>Pts predit</th><th>Rang reel</th><th>Pts reel</th><th>Δ rang</th><th>Δ pts</th></tr>
      ...
      ?>
      <tr<?=$style?>>
        <td><?=h($r['team'])?></td><td><?= $r['pr']?:'—' ?></td><td><?= $r['pp']!==null?(int)$r['pp']:'—' ?></td>
        <td><?= $r['rr']?:'—' ?></td><td><?= $r['rp']!==null?(int)$r['rp']:'—' ?></td>
        <td><?=$drTxt?></td><td><?=$dpTxt?></td>
      </tr>
      <?php endforeach; ?>
    </table>
  </div>
